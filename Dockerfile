...
COPY brainzgraphinator/ ./brainzgraphinator/

RUN uv venv /app/.venv && \
    uv pip install --python /app/.venv/bin/python "/wheels/$(basename "$(find /wheels -name '*.whl' -print -quit)")[neo4j,rabbitmq,otel]" && \
...
LABEL org.opencontainers.image.title="musicbrainz-graph-enricher" \
      ...
      org.opencontainers.image.documentation="https://github.com/groovemap-music/musicbrainz-graph-enricher/blob/main/README.md" \
      org.opencontainers.image.source="https://github.com/groovemap-music/musicbrainz-graph-enricher" \
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=60s --retries=3 \
    CMD ["/app/.venv/bin/python", "-c", "from urllib.request import urlopen; urlopen('http://127.0.0.1:8011/health', timeout=5).read()"]

USER 1000:1000
ENTRYPOINT ["/app/.venv/bin/musicbrainz-graph-enricher"]
