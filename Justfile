...
test:
    uv run pytest -m "not integration" --cov=brainzgraphinator --cov-report=term-missing --cov-report=xml
...
image: prepare-runtime-wheel
    ...
    docker run --rm --entrypoint /app/.venv/bin/python musicbrainz-graph-enricher:local -c 'import brainzgraphinator.brainzgraphinator'
    test "$(docker run --rm --entrypoint /usr/bin/id musicbrainz-graph-enricher:local -u):$(docker run --rm --entrypoint /usr/bin/id musicbrainz-graph-enricher:local -g)" = "1000:1000"
